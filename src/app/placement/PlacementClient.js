...
const PlacementClient = ({ initialYears = [], initialStars = [], initialGallery = [], initialDrives = [] }) => {
    const [activeTab, setActiveTab] = useState(initialYears.length > 0 ? initialYears[0].year : '');
    const [loading, setLoading] = useState(false); // No loading due to SSR

    // --- DYNAMIC DATA STATE ---
    const [years, setYears] = useState(initialYears);
    const [starAchievers, setStarAchievers] = useState(initialStars);
